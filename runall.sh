...
archivo=prueba.txt #.txt with the name of all NAMELIST inputs for the tool
generador=frescoPRC_no_j_var #calling frescoPRC tool to generate the inputs for each target in $archivo
...
while read este; do
  ini=${este%%.*}
  final=$ini.new.xsec_3d_no_j_var	
  #echo $este >> $final
  ...
  rm -f $lista
  rm -f fort*
  rm -f fresco-* #Borrar todos los inputs entre un target y el siguiente
  rm -f DOMEIC16*
done < $archivo
